...
def netG(frames, noise, num_actions, useNoise):
    ...
    conv1 = tcl.conv2d(frames, 32, 5, 2, activation_fn=tf.nn.relu, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_conv1')
    conv1 = tcl.batch_norm(conv1)
    print('conv1:',conv1)

    conv2 = tcl.conv2d(conv1, 64, 5, 2, activation_fn=tf.nn.relu, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_conv2')
    conv2 = tcl.batch_norm(conv2)
    print('conv2:',conv2)
    
    conv3 = tcl.conv2d(conv2, 128, 5, 2, activation_fn=tf.nn.relu, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_conv3')
    conv3 = tcl.batch_norm(conv3)
    print('conv3:',conv3)

    conv4 = tcl.conv2d(conv3, 256, 5, 2, activation_fn=tf.nn.relu, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_conv4')
    conv4 = tcl.batch_norm(conv4)
    print('conv4:',conv4)
    
    conv5 = tcl.conv2d(conv4, 512, 5, 2, activation_fn=tf.nn.relu, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_conv5')
    conv5 = tcl.batch_norm(conv5)
    print('conv5:',conv5)

    conv6 = tcl.conv2d(conv5, 512, 5, 2, activation_fn=tf.nn.relu, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_conv6')
    conv6 = tcl.batch_norm(conv6)
    print('conv6:',conv6)
    
    conv7 = tcl.conv2d(conv6, 512, 5, 2, activation_fn=tf.nn.relu, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_conv7')
    conv7 = tcl.batch_norm(conv7)
    print('conv7:',conv7)
    
    conv8 = tcl.conv2d(conv7, 512, 5, 2, activation_fn=tf.nn.relu, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_conv8')
